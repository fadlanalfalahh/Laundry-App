class StrukModel {
  ...
  factory StrukModel.fromTransaksi(Map<String, dynamic> master, dynamic details) {
    return StrukModel(
      ...
      uangDiterima: int.tryParse(master['uang_diterima']?.toString() ?? '0') ?? 0,
  ...
  factory StrukModel.fromPengambilan(Map<String, dynamic> master, List<dynamic>? details) {
    return StrukModel(
      namaPelanggan: master['nama_pelanggan'] ?? '',
      createdBy: master['created_by'] ?? '',
      statusBayar: master['status_bayar'] ?? '',
