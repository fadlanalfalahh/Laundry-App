...
class _DaftarCucianPageState extends State<DaftarCucianPage> with SingleTickerProviderStateMixin {
  ...
  void _tambahData(bool isPaket) {
    ...
    final durasiOptions = List.generate(7, (i) => {'label': '${i + 1} Hari', 'value': 24 * (i + 1)})
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                final harga = int.tryParse(controllerHarga.text.replaceAll('.', '').trim()) ?? 0;
  ...
  void _editData(LayananModel item, bool isPaket) {
    ...
    final controllerHarga = TextEditingController(
      text: NumberFormat.currency(locale: 'id_ID', symbol: '', decimalDigits: 0).format(item.harga),
    );
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (result['success'] == true) {
                  await _loadData();
                  setState(() {
                    isPaket ? _selectedPaketIndex = null : _selectedSatuanIndex = null;
                  });
                  Navigator.pop(context);
  ...
  // Tampilkan data Satuan dalam grup (accordion) per durasi
        ...
        return Card(
          ...
          child: Theme(
            data: Theme.of(context).copyWith(
              // Hilangkan garis atas/bawah bawaan ExpansionTile
              dividerColor: Colors.transparent,
            ),
            child: ExpansionTile(
              // Opsional: rapihkan padding & hilangkan outline shape
  ...
  Widget _buildList(List<LayananModel> data, bool isPaket) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (jam > 0) durasiText += '${durasiText.isNotEmpty ? ' ' : ''}$jam Jam';
        ...
        return GestureDetector(
          onTap: () {
            setState(() {
              if (isPaket) {
                _selectedPaketIndex = _selectedPaketIndex == index ? null : index;
              } else {
                _selectedSatuanIndex = _selectedSatuanIndex == index ? null : index;
              }
            });
          },
          child: Card(
    ...
    return Theme(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          controller: _tabController,
          children: [
            _buildList(_paket, true),
            _buildSatuanGrouped(_satuan),
          ],
        ),
