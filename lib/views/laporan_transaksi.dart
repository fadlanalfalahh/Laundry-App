...
class _LaporanTransaksiPageState extends State<LaporanTransaksiPage> {
  ...
  Future<Uint8List> _generatePdf() async {
    ...
    final totalKeseluruhan = _laporan.fold<int>(
      0,
          (sum, item) {
        if (item.statusBayar.toLowerCase() == 'batal') return sum; // lewati
                  ...
                  return pw.TableRow(
                    ...
                    children: [
                      ...
                      pw.Container(
                        alignment: pw.Alignment.center,
                        padding: const pw.EdgeInsets.all(4),
                        child: pw.Text(lap.noTransaksi, style: const pw.TextStyle(fontSize: 10)),
                      ),
                      // Pelanggan
                      pw.Container(
                        alignment: pw.Alignment.center,
                        padding: const pw.EdgeInsets.all(4),
                        child: pw.Text(namaDepanPelanggan, style: const pw.TextStyle(fontSize: 10)),
                      ),
                      // Kasir
                      pw.Container(
                        alignment: pw.Alignment.center,
                        padding: const pw.EdgeInsets.all(4),
                        child: pw.Text(kasirDepan, style: const pw.TextStyle(fontSize: 10)),
                      ),
                      // Tanggal Transaksi
                      pw.Container(
                        alignment: pw.Alignment.center,
                        padding: const pw.EdgeInsets.all(4),
                        child: pw.Text(
                          _dfDate.format(DateFormat('dd-MM-yyyy').parse(lap.createdAt)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Laporan Transaksi", style: TextStyle(color: Colors.white)),
              ...
              child: _laporan.isEmpty
                ...
                separatorBuilder: (_, __) => const SizedBox(height: 10),
                  itemBuilder: (context, i) {
                    ...
                    return Card(
                      color: isBatal ? Colors.red[100] : null, // card jadi merah kalau batal
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            // === Header: Nomor urut & No Transaksi ===
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text("No. ${i + 1}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                Text(lap.noTransaksi, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                              ],
                            ),
                            const Divider(height: 12, thickness: 1),

                            // === Detail transaksi pakai Table ===
                            Table(
                              columnWidths: const {
                                0: IntrinsicColumnWidth(), // label
                                1: FixedColumnWidth(12),   // titik dua
                              ...
                              children: [
                                ...
                                TableRow(children: [
                                  buildCellLabel("Tanggal Transaksi"),
                                  const Text(":"),
                                  Text(_dfTanggalFull.format(DateFormat('dd-MM-yyyy').parse(lap.createdAt))),
                                ]),
                                TableRow(children: [
                                ...
                                TableRow(children: [
                                  ...
                                  Text(
                                    isBatal ? '-' : _rupiah.format(lap.totalHarga),
                                    style: TextStyle(
                                      color: isBatal ? Colors.red : Colors.black,
                                      fontWeight: FontWeight.w600,
                                    ),
                                  ),
                                ]),
                                ...
                                TableRow(children: [
                                  ...
                                  const Text(":"),
                                  Text(capitalizeWords(lap.jenisLayanan)),
                                ]),
                                if (lap.detail.isNotEmpty)
                                  TableRow(children: [
                                    buildCellLabel("Detail Layanan"),
                                    const Text(":"),
                                    Text(lap.detail.map((d) => d.namaLayanan).join(', ')),
                                  ]),
                              ],
                            ),
                          ],
