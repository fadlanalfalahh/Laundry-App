...
class DashboardPage extends StatelessWidget {
  ...
  Future<void> _logout(BuildContext context) async {
    ...
    // Hapus hanya kunci sesi
    await prefs.remove('is_logged_in');
    await prefs.remove('nama_user');
    await prefs.remove('role');
    await prefs.remove('username');

    // Feedback cepat agar terasa respon
    tampilkanAlert(context, "Berhasil logout");

    // Jika Flutter kamu belum support context.mounted, baris ini bisa dihapus
    if (!context.mounted) return;

    // Buang semua route dan kembali ke Login
  ...
  static List<String> _labelsForRole(String roleRaw) {
    ...
    // default: role kosong/tidak dikenal -> seperti staff
    return staffOnly;
  }
            ...
            child: Padding(
              ...
              child: GridView.count(
                ...
                children: _labelsForRole(role)
                    .map((label) => _buildMenuCard(context, _iconFor(label), label))
                    .toList(),
              ),
            ),
          ),
  ...
  static Widget _buildMenuCard(
    ...
    return GestureDetector(
      onTap: () {
        ...
        // Menu lainnya bisa ditambahkan di sini nanti
      },
      child: Container(
