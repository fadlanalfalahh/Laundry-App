...
class _TransaksiPageState extends State<TransaksiPage> {
  ...
  int get uangDiterima {
    final digitsOnly = _uangDiterimaController.text.replaceAll(RegExp(r'[^0-9]'), '');
  ...
  void _simpanTransaksi() async {
    ...
    if (nama.isEmpty ||
        _layananDipilih.isEmpty || // cek langsung daftar layanan, bukan _layananTerpilih
        _statusBayar == null ||
    ...
    if (_statusBayar == 'Lunas' && uangDiterima < total.toInt()) {
      tampilkanAlert(context, 'Uang diterima kurang dari Grand Total. Transaksi ditolak.');
      return;
    }
    ...
    final transaksi = TransaksiModel(
      ...
      jenisLayanan: '', // kosong karena multi layanan
      beratLayanan: '',
      durasiLayanan: 0,
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        // bersihkan form — boleh juga dihapus jika tak dibutuhkan
        ...
        // Tutup keyboard/overlay dulu biar Navigator tidak bentrok
        if (!mounted) return;
        FocusScope.of(context).unfocus();

        // Beri jeda singkat agar frame settle (hindari race dengan input method / overlay)
        ...
        final details = data['details'] != null
            ...
            : <Map<String, dynamic>>[];
        debugPrint("=== HASIL API TRANSAKSI ===");
    ...
    _namaController.addListener(() {
      ...
      if (text != formatted) {
        _namaController.value = _namaController.value.copyWith(
          text: formatted,
          // Biar kursor tetap di akhir; sederhana & stabil
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () {
                    if ((_jenisLayananDipilih == 'Kiloan' && _layananTerpilih != null && _beratController.text.isNotEmpty) ||
                        (_jenisLayananDipilih == 'Satuan' && _layananTerpilih != null && _durasiDipilih != null)) {

                      final layananBaru = {
                        'jenis': _jenisLayananDipilih,
                        'nama': _layananTerpilih?.nama ?? '',
                        'berat': _jenisLayananDipilih == 'Kiloan' ? _beratController.text : '1',
                        'durasi': _durasiDipilih ?? (_layananTerpilih?.durasi ?? 0),
                ...
                if (_layananDipilih.isNotEmpty)
                  Column(
                    ...
                    children: [
                      ...
                      ..._layananDipilih.map((layanan) => Card(
                        child: ListTile(
                          title: Text('${layanan['jenis']} - ${layanan['nama']}'),
                          subtitle: Text(
                          ...
                          trailing: Text(
                            (() {
                              final bool isKiloan = layanan['jenis'] == 'Kiloan';
                              final int harga = (layanan['harga'] as int);
                              final double berat = (layanan['berat'] is num)
                                  ? (layanan['berat'] as num).toDouble()
                                  : double.tryParse(layanan['berat'].toString()) ?? 1.0;
                              final int lineTotal = isKiloan ? (harga * berat).round() : harga;
                  ...
                  ...[
                    ...
                    TextField(
                      ...
                      onChanged: (value) {
                        ...
                        final formatted = NumberFormat.decimalPattern('id_ID').format(angka);
  ...
  Widget _buildTextField(TextEditingController controller, String label) {
    ...
    return TextField(
      textCapitalization: isNama ? TextCapitalization.words : TextCapitalization.none,
    ...
    return DropdownButtonFormField<String>(
      ...
      onChanged: (value) {
        setState(() {
          ...
          // RESET status pembayaran dan uang diterima
          _statusBayar = null;
          _uangDiterimaController.clear();
        });
  ...
  List<Widget> _buildLayananKiloan() {
    return [
      _isLoadingLayanan
        ...
        items: _listLayanan.map((layanan) => DropdownMenuItem(
          value: layanan,
      ...
      // Tampilkan Berat (kg) HANYA jika layanan sudah dipilih
  ...
  List<Widget> _buildLayananSatuan() {
    ...
    return [
      DropdownButtonFormField<String>(
        value: namaList.contains(_layananSatuanDipilih) ? _layananSatuanDipilih : null,
        decoration: const InputDecoration(
          labelText: 'Layanan Satuan',
          border: OutlineInputBorder(),
        ),
        hint: const Text('Pilih Layanan Satuan'),
        items: namaList.map((nama) => DropdownMenuItem(
          value: nama,
          child: Text(nama),
        )).toList(),
        onChanged: (value) {
          setState(() => _layananSatuanDipilih = value);
          if (value != null) _loadDurasi(value); // trigger durasi
        },
      ),
      const SizedBox(height: 12),
      ...
      if (_durasiTersedia.isNotEmpty)
        DropdownButtonFormField<int>(
          ...
          items: _durasiTersedia.map((d) => DropdownMenuItem(
            value: d,
          ...
          onChanged: (value) async {
            ...
            if (_layananSatuanDipilih != null && value != null) {
              final harga = await LayananService.getHargaDariLayananDanDurasi(_layananSatuanDipilih!, value);
