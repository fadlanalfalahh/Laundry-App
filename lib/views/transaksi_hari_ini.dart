...
void tampilkanAlert(BuildContext context, String pesan) {
  ScaffoldMessenger.of(context).clearSnackBars(); // Hapus snack sebelumnya (jika ada)
...
class _TransaksiHariIniPageState extends State<TransaksiHariIniPage>
  ...
  Future<List<Map<String, dynamic>>> _loadData(String? status) async {
    // status: 'lunas' atau 'belum lunas' (mengikuti enum di DB)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('Transaksi Hari Ini', style: TextStyle(color: Colors.white)),
    ...
    return RefreshIndicator(
      ...
      child: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          // [PATCH] Filter by search query (no_transaksi / nama_pelanggan)
          final q = _query;
          final filtered = (q.isEmpty)
              ? items
              : items.where((it) {
            final noTrans = (it['no_transaksi'] ?? '').toString().toLowerCase();
            final nama    = (it['nama_pelanggan'] ?? '').toString().toLowerCase();
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final total   = int.tryParse((it['total_harga'] ?? '0').toString()) ?? 0;
              final uang    = int.tryParse((it['uang_diterima'] ?? '0').toString()) ?? 0;
              final kembali = int.tryParse((it['kembalian'] ?? '0').toString()) ?? 0;

              // Sisa pembayaran = total - (uang diterima - kembalian). Tidak boleh negatif.
              final int sisa = (total - (uang - kembali)) > 0 ? (total - (uang - kembali)) : 0;
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          onPressed: () async {
                            final idMaster = int.tryParse(it['id_master'].toString()) ?? 0;
                            ...
                            if (res['success'] == true) {
                              ...
                              if (!mounted) return;
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text(res['message'] ?? 'Gagal mengambil struk')),
                              );
                            }
                          },
                          icon: const Icon(Icons.visibility, color: Colors.blue, size: 40),
                          tooltip: 'Lihat Struk',
                        ),
                        if (_role == 'admin') // 👈 hanya admin bisa batal
                          IconButton(
                            onPressed: () async {
                              final idMaster = int.tryParse(it['id_master'].toString()) ?? 0;
                              ...
                              if (confirm == true) {
                                final res = await TransaksiService.cancelTransaksi(idMaster, _role);
                                if (res['success'] == true) {
                                  if (!mounted) return;
                                  tampilkanAlert(context, "Transaksi berhasil dibatalkan.");
                                  setState(() {}); // refresh list
                                } else {
                                  tampilkanAlert(context, res['message'] ?? "Gagal membatalkan transaksi.");
                                }
                              }
                            },
